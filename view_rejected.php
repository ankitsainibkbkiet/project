...
function fetchApplicationsFromDb($dbname) {
    ...
    // Fetch data from register table where shortlisted = 0
    $sql = "SELECT User_id, email, phone, application_no, shortlisted FROM register WHERE shortlisted = 0";
...
function updateShortlistedStatus($dbname, $shortlistedCandidates) {
    ...
    // Update the shortlisted status to 1 for selected candidates
...
if (isset($_GET['adv'])) {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        header("Location: view_rejected_applications.php");
        exit();
    }
}
?>
        ...
        <?php if (isset($fields) && isset($rows)) { ?>
            ...
            <form method="POST">
                <table class="table table-bordered">
                    <thead>
                        <tr>
                            <th class="checkbox-column">Shortlist The Candidate</th>
                ...
                <input type="button" value="Back" onclick="location.href='view_rejected_applications.php'">
                 </div>
            </form>
        <?php } else { ?>
