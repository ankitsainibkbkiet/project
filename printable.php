...
function confirmFinalize() {
    var checkbox = document.getElementById('termsCheckbox');
    if (!checkbox.checked) {
        alert("You must agree to the terms and conditions before finalizing.");
        return;
    }

    var result = confirm("Are you sure you are submitting your application? You won't be able to edit the form again.");
    if (result) {
        ...
        xhr.onload = function () {
            if (xhr.status === 200) {
                ...
                if (response.status === 'success') {
                    ...
                    window.location.href = 'logout.php'; // Optionally reload the page
                } else {
                    alert('Error finalizing application: ' + response.message);
                }
            } else {
                alert('Request failed.');
            }
        };
        xhr.send('action=finalize');
    }
}
...
function printPage() {
    var checkbox = document.getElementById('termsCheckbox');
    if (!checkbox.checked) {
        alert("You must agree to the terms and conditions before printing.");
        return;
    }
    window.open('printdoc.php', '_blank');
}

window.onload = function() {
    document.getElementById('termsCheckbox').addEventListener('change', validateActions);
    validateActions(); // Initial state of buttons based on checkbox
}
</script>
...
<div class="container">
    ...
    if ($result->num_rows > 0) {
        while($row = $result->fetch_assoc()) {
            ...
            echo '<tr><td class="header">Disability Certificate: <a href="'. $row['disability_certificate'] .'" target="_blank">View</a></td><td class="header">corrospondance Address: '. $row['corrospondance_address'] . '</td><td class="header">Permanent Address: '. $row['permanent_address'] . '</td></tr>';
            echo '</table>';
            echo '<table>';
            echo '<tr><td class="header" colspan="2">corrospondance City Name: '. $row['corrospondance_city'] . '</td><td class="header" colspan="2">Permanent City Name: '. $row['permanent_city'] . '</td></tr>';
            echo '</table>';
        }
    } else {
    ...
    <div class="terms-checkbox">
        ...
        <label for="termsCheckbox">I Agree With All The Terms And Conditions</label>
    </div>

    <button id="printButton" class="print-button" onclick="printPage()">Print this page</button>
    <button id="finalizeButton" class="finalize-button" onclick="confirmFinalize()">Finalize</button>
</div>
</div>
