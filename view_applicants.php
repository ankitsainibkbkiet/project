...
function fetchApplicationsFromDb($dbname) {
    ...
    // Fetch data from register table where is_finalized = 1
    $sql = "SELECT User_id, email, phone, application_no, shortlisted FROM register WHERE is_finalized = 1";
    ...
    while ($registerRow = $registerResult->fetch_assoc()) {
        ...
        $values = [
            ...
            'shortlisted' => $registerRow['shortlisted'], // This is necessary to determine if the checkbox should be disabled
            'user_id' => $registerRow['User_id'], // This is necessary for updating the shortlisted status
        ];
...
function updateShortlistedStatus($dbname, $shortlistedCandidates) {
    ...
    foreach ($shortlistedCandidates as $userId) {
        $sql = "UPDATE register SET shortlisted = 1 WHERE User_id = '$userId'";
        $conn->query($sql);
    }
...
if (isset($_GET['adv'])) {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        echo "<p class='alert alert-success'>Shortlisted candidates have been updated successfully.</p>";
        ...
        <?php if (isset($fields) && isset($rows)) { ?>
            ...
            <form method="POST">
                <table class="table table-bordered">
                    <thead>
                        <tr>
                            <th class="checkbox-column">Shortlisted?</th>
                        ...
                        <?php foreach ($rows as $row) { ?>
                            <tr>
                                <td class="checkbox-column">
                                    ...
                                    <?php if ($row['shortlisted'] == 1) echo 'disabled checked'; ?>>
                                </td>
                                <?php foreach ($fields as $field) { ?>
                ...
                <button type="submit" class="btn btn-primary">Submit</button><div class="form-section">
                <input type="button" value="Back" onclick="location.href='view_all_applications.php'">
                 </div>
            </form>
        <?php } else { ?>
